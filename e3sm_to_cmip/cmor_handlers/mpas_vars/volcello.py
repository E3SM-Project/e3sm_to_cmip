...
def handle(infiles, tables, user_input_path, **kwargs):
    ...
    with mpas.open_mfdataset(timeSeriesFiles, variableList) as dsIn:
        ds[VAR_NAME] = dsIn.timeMonthly_avg_layerThickness.where(cellMask3D, 0.0)
    ...
    area_b = xarray.DataArray(
        data=area_b, dims=("lat", "lon"), coords=(ds.coords["lat"], ds.coords["lon"])
    )
