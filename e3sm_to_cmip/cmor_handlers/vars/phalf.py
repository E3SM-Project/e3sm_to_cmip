...
try:
    ...
    print("The phalf variable handler requires both cdms2 and cdutil")
    raise e
...
def write_data(varid, data, timeval, timebnds, index, **kwargs):
    ...
    cmor.write(
        data["ips"], data["ps"], time_vals=timeval, time_bnds=timebnds, store_with=varid
    )
...
def handle(infiles, tables, user_input_path, **kwargs):  # noqa: C901
    ...
    for index in range(num_files_per_variable):
        ...
        for var_name in RAW_VARIABLES:
            ...
            msg = "{name}: loading {variable}".format(name=VAR_NAME, variable=var_name)
            logger.info(msg)
            ...
            new_data = {
                i: f(i) for i in ["hyai", "hybi", "hyam", "hybm"] if i in f.variables
            }
        ...
        axes = [
            {str("table_entry"): "time2", str("units"): data["time2"].units},
            {
        ...
        cmor.zfactor(
            zaxis_id=axis_ids[1], zfactor_name="p0", units="Pa", zfactor_values=100000
        )
        ips = cmor.zfactor(
            zaxis_id=axis_ids[1], zfactor_name="ps2", axis_ids=[0, 2, 3], units="Pa"
        )
        ...
        msg = "{}: time {:1.1f} - {:1.1f}".format(
            VAR_NAME, data["time_bnds"][0][0], data["time_bnds"][-1][-1]
        )
        logger.info(msg)
