...
def main():
    ...
    if _args.get("handlers"):
        ...
        handlers_path, _ = os.path.split(os.path.abspath(cmor_handlers.__file__))
    ...
    if len(handlers) == 0:
        ...
        sys.exit(1)
    if _args.get("info"):
        print_var_info(handlers, freq, input_path, tables_path, _args.get("info_out"))
        sys.exit(0)
    ...
    if custom_metadata:
        add_metadata(file_path=output_path, var_list=var_list, metadata=custom_metadata)
