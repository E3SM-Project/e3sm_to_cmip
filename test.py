...
def test(vars: List, cmp_branch: str, input: Path, output_path: Path, cwl_path: Path, map_path: str, vrt_map: str, metadata: str):
    ...
    issues = []
    tables = [x for x in Path(cmp_output_path, 'CMIP6/CMIP/E3SM-Project/E3SM-1-0/piControl/r1i1p1f1/').glob("**")]
    for table in tables:
        cmp_branch_path = table.glob("**")
        src_branch_path = Path(src_output_path, 'CMIP6/CMIP/E3SM-Project/E3SM-1-0/piControl/r1i1p1f1/', table.name)
...
def main():
    ...
    parser.add_argument(
        ...
        help=f'metadata file to use in the conversion process, default path is {default_metadata_path}')
